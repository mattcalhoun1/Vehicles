...
void VehicleServos::init (int startingRotations[], int startingTilts[]) {
  ...
  //servoDriver = Adafruit_PWMServoDriver(VEHICLE_SERVOS_PCA_ADDRESS);
  ...
  channelsTilt[1] = 3; // not connected yet
  vehicleFore[1] = 170;
  vehicleAft[1] = 25;
  ...
  Serial.println("All cameras set to defaults");
...
void VehicleServos::setVehicleRelativeRotation (int angle, int cameraNum) {
  if (angle > vehicleRelativeMax[cameraNum] || angle < vehicleRelativeMin[cameraNum]) {
    Serial.println("ERROR: Out of range rotation angle " + String(angle) + " received for camera " + String(cameraNum));
    return;
  }
...
void VehicleServos::setCameraRotation(int angle, int cameraNum) {
  ...
  //Serial.println("New pulse width for camera " + String(cameraNum) + " rotation " + String(channelsRotation[cameraNum]) + " = " + String(newPulseWidth));
  int result = servoDriver.setPWM(channelsRotation[cameraNum], 0, newPulseWidth);
  if (result != 0) {
    Serial.println("Received nonzero result: " + String(result) + " from PWM set on pca motor: " + String(channelsRotation[cameraNum]));
  }
...
void VehicleServos::setCameraTilt(int angle, int cameraNum) {
  if (cameraNum < 1) {
    int newPulseWidth = getPulseWidth(angle, CAM_SERVO_MIN_TILT, CAM_SERVO_MAX_TILT);
    //Serial.println("New pulse width for camera " + String(cameraNum) + " tilt " + String(channelsTilt[cameraNum]) + " = " + String(newPulseWidth));
    int result = servoDriver.setPWM(channelsTilt[cameraNum], 0, newPulseWidth);
    if (result != 0) {
      Serial.println("Received nonzero result: " + String(result) + " from PWM set on pca motor: " + String(channelsTilt[cameraNum]));
    }
