...
{
  // Set up left sensor
  pinMode(ECHO_PIN_LEFT, INPUT); //Ultrasonic module initialization
  pinMode(TRIG_PIN_LEFT, OUTPUT);

  // set up right sensor
  pinMode(ECHO_PIN_RIGHT, INPUT); //Ultrasonic module initialization
...
Measurement Obstructions::getDistanceInRange(float angle, float angleTolerance, float expectedDistance, float distanceTolerance, unsigned long maxAge)
{
  ...
  Serial.println("Want measurement for angle " + String(angle) + " index " + getLidarIndex(angle) + " near " + String(expectedDistance) + " mm");
...
void Obstructions::collectMeasurements(unsigned long timeoutMillis)
{
  ...
  while ((measurementTime - startTime) < timeoutMillis) {
    if (IS_OK(lidar.waitPoint())) {
      ...
      if (lidarAngle >= 0.0 && lidarAngle <= 360.0) {
        int measIndex = getLidarIndex(lidarAngle);
        lidarMeasurements[measIndex] = lidarDistance;
        lidarTimes[measIndex] = measurementTime;

        if (measIndex < 0 || measIndex > 720) {
          Serial.println("!! Array out of bounds for angle " + String(lidarAngle) + ": " + String(measIndex));
        }
      }
    }
    else {
      Serial.println("Restarting Lidar");
      startLidar();
    }
  }
...
int Obstructions::getDistanceBumperLeft()
{
  ...
  digitalWrite(TRIG_PIN_LEFT, LOW);
  return ((unsigned int)pulseIn(ECHO_PIN_LEFT, HIGH) / 58);
}

int Obstructions::getDistanceBumperRight()
{
  unsigned int tempda_x = 0;
  ...
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN_RIGHT, LOW);
  return ((unsigned int)pulseIn(ECHO_PIN_RIGHT, HIGH) / 58);
}
